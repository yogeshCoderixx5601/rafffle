...
function processTagsParam(
  ...
  console.debug(`Processing tags parameter: ${tagsValue}`);
...
function processSortParam(finalQuery: FinalQuery, sortParam: string): void {
  console.debug("Processing sort parameter:", sortParam);
...
function processWhereParam(
  ...
  console.debug(`Processing where parameter: key=${key}, value=${value}`);
  ...
  if (key === "content" || key === "search") {
    ...
    switch (value) {
      ...
      case "content":
        ...
        break;
      case "content-type":
        const tags = params.q.split("|").map((tag: string) => tag.trim());
    ...
    if (isObjectId) {
      ...
      finalQuery.find["attributes.value"] = new RegExp(value, "i");
    } else {
...
export default function convertParams(
  ...
  console.debug("Converting URL parameters to Mongoose query");
  ...
  Object.keys(params).forEach((key) => {
    if (key.includes("_sort")) {
      ...
      finalQuery.find[key] = params[key] === 'true';
    } else {
  ...
  console.debug(finalQuery, "Final query before return");
  return finalQuery;
}
