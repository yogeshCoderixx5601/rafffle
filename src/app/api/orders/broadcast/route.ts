...
export async function POST(
  ...
  try {
    ...
    const listings = await Raffle.find({
      statu: true,
      utxo_id: { $in: outputs },
    ...
    if (txid) {
      if (listings) {
        ...
        console.log(Raffle, "updated runes utxo");
        const updatedUtxos = await Raffle.find({ utxo_id: { $in: outputs } });
        console.log(updatedUtxos,"UPDATED UTXOS AFTER BUY")
      }
